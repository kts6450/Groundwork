...
화면은 검증 → 업종 분포 → 기획안 → 개업 이후 순서로 펼쳐진다. 지도 위 표시만 아직 없다(Kakao 키 필요).

업종 분포는 `GET /backend/distribution?sido=&sgg=`가 준다. 절대 수가 아니라 전국 구성비 대비
배수(`ratio`)가 핵심이다. 인구가 많은 구는 모든 업종이 많아 절대 수로는 차이가 안 보인다.
